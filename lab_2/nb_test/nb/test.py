...
def substract_data():
    f = open('/media/simon/Study/3-up/AI/lab_2/test/zhouwuxiawuKNN/train_set.csv','r')
    line = f.readline()
    line = f.readline()
    word_list = []
    label = []
...
def classify0(inX,dataSet,labels,k,row):
    # test_matrix = [[inX] for i in range(row)]
    train_data = array(dataSet)
    test_data = tile(array(inX),(row,1))
    # diffMat = [[0 for i in range(col)] for i in range(row)]
    # for i in range(row):
    #     for j in range(col):
    #         diffMat[i][j] = inX[j] - dataSet[i][j]
...
def classify(data_list,word_list,row,col,label,train_data):
    [test_data,answer] = get_test_data('/media/simon/Study/3-up/AI/lab_2/test/zhouwuxiawuKNN/test_set.csv')
    onehot = []
    tf = []
    tf_idf = []
...
predict = classify(data_list,word_list,row,col,label,tf)
print(predict)
